...
$env:PYTHONUTF8 = '1'
$Python = Invoke-VibePython -Python $BootstrapPython -Arguments @('-B', "$Root\scripts\bootstrap_vibe_python.py", '--codex-home', $ConfigHome, '--requirements', "$Root\runtime\scripts\requirements.txt", '--env-name', $(if ($env:VIBE_CONDA_ENV) { $env:VIBE_CONDA_ENV } else { 'vibe-coding' }), '--no-create', '--print-python') -CaptureOutput
if ($script:VibeExitCode -ne 0) { exit $script:VibeExitCode }
Invoke-VibePython -Python $Python -Arguments (@('-B', "$Root\scripts\global_installer.py", 'uninstall') + $args)
