...
9. **Goal 控制器按客户端声明，完成出口唯一。** 线程级运行、暂停、恢复、预算和 continuation 由当前客户端的指定控制器承担：Codex 客户端使用原生 `/goal`；OpenCode 客户端使用 `@prevalentware/opencode-goal-plugin`（固定版本，见能力对齐矩阵）。无论由谁控制线程，Project Goal 都保存业务完成契约、`vibe goal` 证据门禁都是唯一完成出口，控制器状态永不作为完成依据。
