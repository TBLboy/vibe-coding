...
2. Run `scripts/update_workflow.py --dry-run` against `${OPENCODE_CONFIG_DIR:-$HOME/.config/opencode}`.
3. Review changed and locally modified files.
4. If safe, run with `--apply`.
5. Confirm the runtime assets and templates exist, then restart OpenCode to load the update.
