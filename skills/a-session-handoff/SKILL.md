...
同时运行 `loopctl handoff` 生成 `.project-log/loop/handoff.md`。Handoff 只记录 Project Goal、Task 与 Run 状态；Project Goal 不因会话状态变化而改变。OpenCode 的会话 Goal 由插件控制面提供，不进入 Handoff 的权威字段。
